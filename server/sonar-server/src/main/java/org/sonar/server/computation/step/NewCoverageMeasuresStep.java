...
public class NewCoverageMeasuresStep implements ComputationStep {
  ...
  private void computeMeasure(final Counters counters, Metric metric, NewCoverageMeasureComputer measureComputer) {
    ...
    for (Counter counter : nonEmptyCounters) {
      variationsBuilder.setVariation(counter.getIndex(), measureComputer.compute(counter));
    }
  ...
  /**
   * Holds the {@link Counter}s (one for each Period in a specific {@link PeriodsHolder}) for specific Component.
   */
  ...
  public static final class Counter {
    ...
    private Counter(int index, @Nullable Date date) {
      this.index = index;
      this.date = date;
    }
  ...
  private interface NewCoverageMeasureComputer {
    double compute(Counter counter);
  }
  ...
  private enum NewLinesToCoverComputer implements NewCoverageMeasureComputer {
    ...
    @Override
    public double compute(Counter counter) {
      return (double) counter.getNewLines();
    }
  }
