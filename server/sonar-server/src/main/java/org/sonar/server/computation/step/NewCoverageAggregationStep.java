...
public class NewCoverageAggregationStep implements ComputationStep {
  ...
  private void initVariationBuilders(Component file, VariationBuilders builders) {
    for (Metric metric : metrics) {
      Optional<Measure> rawMeasure = measureRepository.getRawMeasure(file, metric);
      if (!rawMeasure.isPresent()
        || !rawMeasure.get().hasVariations() || rawMeasure.get().getValueType() != Measure.ValueType.NO_VALUE) {
        continue;
      }

      builders.getOrCreateBuilder(metric.getKey()).incrementVariationsBy(rawMeasure.get().getVariations(), periodsHolder);
    }
  }
